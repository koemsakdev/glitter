...
export function BadgeFormDialog({
  ...
  const [color, setColor] = useState(item?.color ?? "#ec4899");

  const validHex = /^#([0-9a-fA-F]{6})$/.test(color);

  function handleSubmit() {
    if (!nameEn.trim() || !nameKm.trim() || !validHex) {
      toast({ title: t("badge.required"), variant: "destructive" });
      return;
    }
    onSubmit({
      nameEn: nameEn.trim(),
      ...
      <div className="flex flex-col">
        <div className="px-6 pb-4 pt-6">
          <h2 className="text-xl font-bold tracking-tight">
            {isEdit ? t("badge.editTitle") : t("badge.add")}
          </h2>
        </div>
        ...
        <div className="sticky bottom-0 mt-6 flex justify-end gap-2 border-t bg-neutral-50 px-6 py-4 dark:bg-neutral-800">
          ...
          >
            ...
            {isEdit ? t("settings.saveChanges") : t("badge.add")}
          </Button>
        </div>
      </div>
    </ResponsiveModal>
