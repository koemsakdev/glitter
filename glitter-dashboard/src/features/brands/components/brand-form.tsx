...
import {useGenerateBrandInfo} from '@/features/ai/use-ai';
import {
    useCreateBrand,
    useUpdateBrand,
...
export function BrandForm({brand, onSuccess, onCancel}: BrandFormProps) {
    ...
    // AI generation — separate mutation instances so the two buttons load independently
    const generateWebsite = useGenerateBrandInfo();
    const generateDescription = useGenerateBrandInfo();
    ...
    async function handleGenerateWebsite() {
        ...
        try {
            const result = await generateWebsite.mutateAsync({
                name,
                field: 'websiteUrl',
            });
            if (!result.value) {
            ...
            form.setValue('websiteUrl', result.value, {
                shouldValidate: true,
                shouldDirty: true,
            });
            toast({
                title: t('common.toast.success'),
                description: t('brand.ai.success'),
                variant: 'success'
            });
        } catch (error) {
            toast({
    ...
    async function handleGenerateDescription() {
        ...
        try {
            ...
            form.setValue('description', result.value, {
                shouldValidate: true,
                shouldDirty: true,
            });
    ...
    async function onSubmit(values: FormValues) {
        ...
        try {
            if (brand) {
                ...
                toast({
                    title: t('common.toast.success'),
                    description: t('brand.edit.success'),
                    variant: 'success'
                });
            } else {
                ...
                toast({
                    title: t('common.toast.success'),
                    description: t('brand.create.success'),
                    variant: 'success'
                });
            }
            onSuccess?.();
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col">
            <div className="space-y-5 px-6 pb-2 pt-2">
                ...
                <FieldGroup>
                        ...
                        render={({field, fieldState}) => (
                            <Field data-invalid={fieldState.invalid}>
                                ...
                                <Input
                                    ...
                                    aria-invalid={fieldState.invalid}
                                />
                                <FieldDescription>{t('brand.field.slug.help')}</FieldDescription>
                                {fieldState.invalid && fieldState.error && (
                                    <FieldError
                                        errors={[
                                            {
                    ...
                    {/* Website with AI sparkle inside */}
                    <Controller
                        name="websiteUrl"
                        control={form.control}
                        render={({field, fieldState}) => (
                            <Field data-invalid={fieldState.invalid}>
                                ...
                                <div className="relative">
                                    ...
                                    <button
                                        ...
                                        disabled={generateWebsite.isPending}
                                        className="absolute right-2 top-1/2 flex size-7 -translate-y-1/2 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-pink-100 hover:text-pink-600 disabled:cursor-not-allowed disabled:opacity-50 dark:hover:bg-pink-500/15 dark:hover:text-pink-300"
                                        aria-label={t('brand.ai.generateWebsite')}
                                        title={t('brand.ai.generateWebsite')}
                                    >
                    ...
                    {/* Description with AI sparkle in bottom-right corner */}
                    <Controller
                        name="description"
                        control={form.control}
                        render={({field}) => (
                            <Field>
                                ...
                                <div className="relative">
                                    <Textarea
                                        ...
                                        className="pr-12 shadow-none focus:shadow-none focus-visible:shadow-none focus:outline-0 focus-visible:outline-none focus:ring-0 focus-visible:ring-0 rounded-lg focus-visible:border-pink-500 dark:focus-visible:border-pink-800"
                                    ...
                                    <button
                                        ...
                                        aria-label={t('brand.ai.generateDescription')}
                                        title={t('brand.ai.generateDescription')}
                                    >
            ...
            {/* Footer */}
            <div
                className="sticky bottom-0 mt-6 flex justify-end gap-2 border-t bg-neutral-50 dark:bg-neutral-800 px-6 py-4">
                {onCancel && (
                    <Button
                        type="button"
                        variant="outline"
